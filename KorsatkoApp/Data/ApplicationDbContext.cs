...
namespace KorsatkoApp.Data {
    public class ApplicationDbContext : IdentityDbContext {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder) {
            ...
            modelBuilder.Entity<Enrolment>()
                .HasOne(s => s.session).WithMany(s => s.Enrolments).HasForeignKey(s => s.SessionId);// one to many relation between Enrolments Tbl and Session tbl
            modelBuilder.Entity<Enrolment>()
                .HasOne(s => s.student).WithMany(s => s.Enrolments).HasForeignKey(s => s.StudentId);// one to many relation between Enrolments Tbl and Students tbl
